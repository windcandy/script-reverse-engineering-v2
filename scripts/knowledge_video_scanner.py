...
def generate_report(viral_videos, path_results):
    ...
    for t in type_order:
        ...
        for v in bucket:
            ...
            lines += [
                ...
                f"- 영상 지표: 조회수 {v['view_count']:,} / 구독자 대비 {v['ratio']}배 / 평균 대비 {v['relative']}배",
    ...
    analysis_placeholder = [
        ...
        "> 후보별로 4단계 분석 + 5가지 감정 방향(충격/궁금증/비교/다큐/사회문화)별 1~3개씩 = 5~15개 변형 + 추천 조합 3개 기록.",
        "> 변형 우선순위: 썸네일 문구 1순위 (시청자 1차 인지·CTR) / 가제 2순위 (검색·SEO). 두 원칙: ① 보존 핵심 키워드 1~3개 가제·썸네일 양쪽 포함 (연관 영상 노출 유지) / ② 원본 표현·구조 변형 (복제 느낌 회피).",
        "> 작성 양식 예시:",
        ">",
        "> ```",
        "> ### {점수}점 — {짧은 후보명}",
        ...
        "> ",
        "> **감정 포인트:** 4~5개",
        "> **알고리즘 키워드 조합:** 핵심 키워드 + 의외성 + 충돌",
        "> **보존 핵심 키워드:** 5~7개",
        "> ",
        "> #### 충격형 (1~3개)",
        "> | # | 가제 | 썸네일 문구 | 역추출 주제 |",
        "> |---|---|---|---|",
        "> | 1 | (15~30자, 보존키워드 포함) | (원본 썸네일 글자수 ±20%, 보존키워드 포함) | ... |",
        "> ... (1~3개)",
        "> ",
        "> #### 궁금증형 (1~3개) / 비교형 (1~3개) / 다큐형 (1~3개) / 사회문화형 (1~3개)",
        "> ... (각 1~3개씩, 총 5~15개. 자율 정지 규칙: 게이트 NO 시 카테고리 종료)",
        "> ",
        "> 서사 유형은 P0에서 결정하지 않음 — P1 §2 퀵서치 후 확정",
        "> ",
        "> #### ⭐ 추천 조합 3가지",
