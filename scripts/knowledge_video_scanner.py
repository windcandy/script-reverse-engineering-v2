...
def generate_report(viral_videos, path_results):
    ...
    lines = [
        ...
        "",
        "> ## 📌 사용 흐름 (잊지 마세요)",
        "> 1. 본 채널 톤 부합 후보 5~10개 1차 선별 (썸네일·문구 시각 비교)",
        "> 2. 메인 클로드에 \"변형 가제 + 역추출 주제 평가해줘\" 요청",
        "> 3. 메인 클로드가 본 보고서 하단 \"## 변형 분석\" 섹션에 결과 자동 추가",
        "> 4. 점수 상위 가제 선택 → 우상향 검증 (VidIQ / Claude in Chrome)",
        "> 5. 선택 확정 → P1 퀵서치 진행",
        "",
